		...
		$('#atrzRfrncY').on('click',function(){
			swal({
				...
				if(result>0){
					...
					let empNo = "${empVO.empNo}";
					let infoData = {"lineInfoNo": lineInfoNo, "empNo": empNo};
					console.log('lineInfo하하',lineInfoNo);
					console.log('empNo하하',empNo);
					$.ajax({
						url : '/edoc/atrzRfrncY',
						type : 'post',
						data : JSON.stringify(infoData),
...
<!-- 					<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 16px; font-weight: normal; vertical-align: middle;" colspan="3"> -->
<%-- 					<c:set var="fileList" value="${list.edocVO.fileGrVO.filesList}" /> --%>
<%-- 					<c:choose> --%>
<%-- 						<c:when test="${empty fileList}"> --%>
<!-- 							<p>첨부된 파일이 없습니다.</p> -->
